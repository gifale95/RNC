...
function create_onset_files_fix(cfg, i_sub, sub_id, type)
...
for this_run = 1: no_runs_first-2
    ...
    for this_trial = 1:	no_trials
        ...
        if ~strcmp(results.trial(this_trial).trial_type, 'catch')
            ...
            this_cat = str2num(this_cat{3}(1:end-4));
            if strcmpi(type, 'exact')
                this_onsets =  results.trial(this_trial).image_on;
            elseif strcmpi(type, 'round')
                this_onsets =  results.trial(this_trial).onset;
            end
            onsets(this_cat) = {horzcat(onsets{this_cat},this_onsets)};
            names(this_cat) = {['Image_' num2str(this_cat)]};
            durations(this_cat) = {repmat(0.5,1,n_rep)};
        end
    end
...
for this_run = no_runs_first-1:no_runs_total
    ...
    load(fullfile(cfg.dirs.data_dir,'results',[sub_id '+'],'fmri',['run', num2str(ct,'%02.f'), '_fmri.mat']));
