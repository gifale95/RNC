function results = sl_between_cond_sim(condition_names,labels,beta_dir,out_dir,cfg)
...
cfg.results.output = 'between_cond_sim';
...
% little hack to make design work -> let is seem like all betas are from
% the same 4 chunks instead of 8 so the design is prepared accordingly 
cfg.files.chunk = repmat(cfg.files.chunk(1:4),length(cfg.files.chunk)/4,1);
