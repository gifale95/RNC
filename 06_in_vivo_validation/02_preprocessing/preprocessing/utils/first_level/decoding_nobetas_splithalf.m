...
function results = decoding_nobetas_splithalf(condition_names,avg_size,beta_dir,beta_avg_dir,out_dir,cfg,i_sub,cfg_sub)
...
if ~isfile(fullfile(beta_dir, 'SPM.mat'))
    copyfile(fullfile(cfg_sub.sub(i_sub).dir,'results','GLM','hrf_fitting','normalized','SPM.mat'), fullfile(beta_dir, 'SPM.mat'))
end 
avg_betas_splithalf(condition_names,avg_size, beta_dir, beta_avg_dir,cfg_sub.sub(i_sub).n_betas, cfg_sub); 
...
%
if cfg_sub.sub(i_sub).n_betas == 8 
...
elseif cfg_sub.sub(i_sub).n_betas > 8 
    % (1) a nx1 vector to indicate what data you want to keep together for 
% cross-validation (typically runs, so enter run numbers)
...
% combine the results from all the iterations 
