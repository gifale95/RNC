...
controversial_betas = controversial_betas(1:15); 
...
cfg.files.chunk = [repmat([1:beta_nr],1 ,60) repmat([beta_nr+1],1,15)]'; % little hack to pretend that all betas come from the same chunks (creates the right design) 
% This creates the leave-one-run-out cross validation design:
cfg.files.label = [ones(1,30*beta_nr), ones(1,30*beta_nr)*2 ones(1,15)]';
cfg.files.xclass = [ones(1,60*beta_nr) ones(1,15)*2];
