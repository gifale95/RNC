...
function results = decoding_nobetas(condition_names,labels,beta_avg_dir,out_dir,cfg,i_sub)
...
for obj_i = 1:max(labels)
    for obj_j = obj_i+1:max(labels)
        
    beta_idxs_i = find(labels==obj_i); 
    beta_idxs_j = find(labels==obj_j);
    min_betas = min([length(beta_idxs_i),length(beta_idxs_j)]);
    sel_beta_idxs_i = datasample(beta_idxs_i,min_betas,'Replace',false); % 7 corresponds to the number of minimum images for one obj
    sel_beta_idxs_j = datasample(beta_idxs_j,min_betas,'Replace',false); % 7 corresponds to the number of minimum images for one obj
    ct = 1; 
    betas = [];
    for i = 1:min_betas
    betas{ct} = fullfile(beta_avg_dir,[condition_names{sel_beta_idxs_i(i)},'_beta_avg.nii']);
    betas{ct+min_betas} = fullfile(beta_avg_dir,[condition_names{sel_beta_idxs_j(i)},'_beta_avg.nii']);
    ct = ct +1;
    end 
    cfg.files.name = betas;
    cfg.files.name
    % assign labels and chunks 
    cfg.files.chunk = repmat([1:min_betas],1,2);
    cfg.files.label = repelem([1:2],min_betas);
