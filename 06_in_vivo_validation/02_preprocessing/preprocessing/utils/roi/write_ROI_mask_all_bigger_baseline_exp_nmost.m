...
hdr = spm_vol(fullfile(cfg.sub(i_sub).dir, 'results','GLM','no_hrf_fitting_explicit_mask','spmT_0004.nii'));
vol = spm_read_vols(hdr); 

% get the brain mask from the GLM
brainmask = spm_read_vols(spm_vol(fullfile(cfg.sub(i_sub).dir, 'results','GLM','no_hrf_fitting_explicit_mask','mask.nii')));
...
maskhdr.fname = fullfile(cfg.sub(i_sub).dir,'roi',[roi_name,'_mask_exp_nmost',num2str(nmost),'.nii']);
