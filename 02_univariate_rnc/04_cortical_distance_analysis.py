...
print('>>> Univariate RNC cortical distance analysis <<<')
print('\nInput parameters:')
...
# Aggregate the absolute baseline difference scores based on cortical distances
...
sorted_h1h2_base_diff = np.zeros((len(all_subjects), len(cortical_distances)))
sorted_l1l2_base_diff = np.zeros((len(all_subjects), len(cortical_distances)))
sorted_h1l2_base_diff = np.zeros((len(all_subjects), len(cortical_distances)))
sorted_l1h2_base_diff = np.zeros((len(all_subjects), len(cortical_distances)))
for s in range(len(all_subjects)):
...
sorted_h1h2_base_diff_diff += sorted_h1h2_base_diff[:,1] - \
	sorted_h1h2_base_diff[:,2]
# l1l2
sorted_l1l2_base_diff_diff = sorted_l1l2_base_diff[:,0] - \
	sorted_l1l2_base_diff[:,1]
sorted_l1l2_base_diff_diff += sorted_l1l2_base_diff[:,1] - \
	sorted_l1l2_base_diff[:,2]
# h1l2
sorted_h1l2_base_diff_diff = sorted_h1l2_base_diff[:,0] - \
	sorted_h1l2_base_diff[:,1]
sorted_h1l2_base_diff_diff += sorted_h1l2_base_diff[:,1] - \
	sorted_h1l2_base_diff[:,2]
# l1h2
sorted_l1h2_base_diff_diff = sorted_l1h2_base_diff[:,0] - \
	sorted_l1h2_base_diff[:,1]
sorted_l1h2_base_diff_diff += sorted_l1h2_base_diff[:,1] - \
	sorted_l1h2_base_diff[:,2]

# Compute the 95% confidence intervals
# CI arrays of shape:
# (CI percentiles × Cortical distances)
ci_sorted_h1h2_base_diff = np.zeros((2, len(cortical_distances)))
ci_sorted_l1l2_base_diff = np.zeros((2, len(cortical_distances)))
ci_sorted_h1l2_base_diff = np.zeros((2, len(cortical_distances)))
ci_sorted_l1h2_base_diff = np.zeros((2, len(cortical_distances)))
...
# Test for decreasing/increasing trends (within-subject significance)
...
for i in tqdm(range(args.n_iter)):
	# Shuffle the univariate responses for each image condition across ROIs
	# (the shuffling is performed independently for each image condition)
	idx = np.vstack([np.random.permutation(len(args.rois)) for _ in range(images)]).T
	shuffled_uni_resp = uni_resp[:,idx,np.arange(images)]
	for s in range(len(all_subjects)):
		for r in range(len(r1)):
			# h1h2
			h1h2_base_diff_null_dist[i,s,r,0] = abs(
				shuffled_uni_resp[s,r1[r],high_1_high_2[s,r]] - \
				np.mean(shuffled_uni_resp[s,r1[r],baseline_images[s,r1[r]]]))
			h1h2_base_diff_null_dist[i,s,r,1] = abs(
				shuffled_uni_resp[s,r2[r],high_1_high_2[s,r]] - \
				np.mean(shuffled_uni_resp[s,r2[r],baseline_images[s,r2[r]]]))
			# l1l2
			l1l2_base_diff_null_dist[i,s,r,0] = abs(
				shuffled_uni_resp[s,r1[r],low_1_low_2[s,r]] - \
				np.mean(shuffled_uni_resp[s,r1[r],baseline_images[s,r1[r]]]))
			l1l2_base_diff_null_dist[i,s,r,1] = abs(
				shuffled_uni_resp[s,r2[r],low_1_low_2[s,r]] - \
				np.mean(shuffled_uni_resp[s,r2[r],baseline_images[s,r2[r]]]))
			# h1l2
			h1l2_base_diff_null_dist[i,s,r,0] = abs(
				shuffled_uni_resp[s,r1[r],high_1_low_2[s,r]] - \
				np.mean(shuffled_uni_resp[s,r1[r],baseline_images[s,r1[r]]]))
			h1l2_base_diff_null_dist[i,s,r,1] = abs(
				shuffled_uni_resp[s,r2[r],high_1_low_2[s,r]] - \
				np.mean(shuffled_uni_resp[s,r2[r],baseline_images[s,r2[r]]]))
			# l1h2
			l1h2_base_diff_null_dist[i,s,r,0] = abs(
				shuffled_uni_resp[s,r1[r],low_1_high_2[s,r]] - \
				np.mean(shuffled_uni_resp[s,r1[r],baseline_images[s,r1[r]]]))
			l1h2_base_diff_null_dist[i,s,r,1] = abs(
				shuffled_uni_resp[s,r2[r],low_1_high_2[s,r]] - \
				np.mean(shuffled_uni_resp[s,r2[r],baseline_images[s,r2[r]]]))
# Aggregate the absolute baseline difference scores based on cortical distance
# Aggregated absolute baseline difference scores null distribution arrays of shape:
# (Iterations × Subjects × ROI cortical distances)
...
for s in range(len(all_subjects)):
	# Test for a decreasing trend for conditions [h1h2, l1l2]
	# h1h2
	...
	# Test for an increasing trend for conditions [h1l2, h1l2]
	# h1l2
...
# Test for a decreasing/increasing trends (between-subject significance)
...
# Correlate the ROI responses across all images. This will result in the
# correlation scores between the in silico fMRI univariate responses of the two
# ROIs within each pariwise ROI comparison.
# Correlation arrays of shape:
# (Subjects × ROI pairwise comparisons)
roi_pair_corr = np.zeros((len(all_subjects), len(r1)))
for s in range(len(all_subjects)):
	for r in range(len(r1)):
		roi_pair_corr[s,r] = pearsonr(uni_resp[s,r1[r]], uni_resp[s,r2[r]])[0]

# Aggregate the correlation scores based on cortical distance
# Aggregated correlation array of shape:
# (Subjects × ROI cortical distances)
sorted_corr = np.zeros((len(all_subjects), len(cortical_distances)))
for d, dist in enumerate(cortical_distances):
	if type(dist) == tuple:
...
# Compute the difference in correlation between cortical distances
sorted_corr_diff = sorted_corr[:,0] - sorted_corr[:,1]
sorted_corr_diff += sorted_corr[:,1] - sorted_corr[:,2]
...
# Test for a decreasing trend (within-subject significance)
# Create the permutation-based null distributions
# Correlation null distribution arrays of shape:
# (Iterations × Subjects × ROI pairwise comparisons)
roi_pair_corr_null_dist = np.zeros((args.n_iter, len(all_subjects),
	len(r1)), dtype=np.float32)
for i in tqdm(range(args.n_iter)):
	# Shuffle the univariate responses for each image condition across ROIs
	# (the shuffling is performed independently for each image condition)
	idx = np.vstack([np.random.permutation(len(args.rois)) for _ in range(images)]).T
	...
	for s in range(len(all_subjects)):
		for r in range(len(r1)):
			roi_pair_corr_null_dist[i,s,r] = pearsonr(
				shuffled_uni_resp[s,r1[r]], shuffled_uni_resp[s,r2[r]])[0]
# Aggregate the correlation scores based on cortical distance
# Aggregated correlation null distribution arrays of shape:
# (Iterations × Subjects × ROI cortical distances)
sorted_corr_null_dist = np.zeros((args.n_iter, len(all_subjects),
	len(cortical_distances)))
for d, dist in enumerate(cortical_distances):
	if type(dist) == tuple:
		sorted_corr_null_dist[:,:,d] = np.mean(
			roi_pair_corr_null_dist[:,:,dist], 2)
	else:
		sorted_corr_null_dist[:,:,d] = roi_pair_corr_null_dist[:,:,dist]
# Compute the difference in correlation between cortical distances
sorted_corr_diff_null_dist = sorted_corr_null_dist[:,:,0] - \
	sorted_corr_null_dist[:,:,1]
sorted_corr_diff_null_dist += sorted_corr_null_dist[:,:,1] - \
	sorted_corr_null_dist[:,:,2]
# Compute the within-subject p-values
sorted_corr_within_subject_pval = np.zeros((len(all_subjects)))
for s in range(len(all_subjects)):
	idx = sum(sorted_corr_diff_null_dist[:,s] > sorted_corr_diff[s])
	sorted_corr_within_subject_pval[s] = (idx + 1) / (args.n_iter + 1) # Add one to avoid p-values of 0
# Compute the within-subject significance
alpha = 0.05
sorted_corr_within_subject_sig = sorted_corr_within_subject_pval < alpha

# Test for a decreasing trend (between-subject significance)
# Compute the probability of observing k or more significant results by chance,
# based on the CDF of the binomial distribution of within-subject significances.
n = len(all_subjects) # Total number of subjects
p = 0.05 # probability of success in each trial
k = sum(sorted_corr_within_subject_sig) # Number of significant subjects
# We use "k-1" because otherwise we would get the probability of observing
# k+1 or more significant results by chance
sorted_corr_between_subject_pval = 1 - binom.cdf(k-1, n, p)
...
stats = {
	...
	'sorted_corr_within_subject_pval' : sorted_corr_within_subject_pval,
	'sorted_corr_within_subject_sig' : sorted_corr_within_subject_sig,
	'sorted_corr_between_subject_pval' : sorted_corr_between_subject_pval,
	}
