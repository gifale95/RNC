...
# =============================================================================
# Load the RSMs
data_dir = os.path.join(args.project_dir, 'retinotopy_effect',
	'nsd_encoding_models', 'imageset-nsd', 'V1-hV4', 'rsms_control_condition-'+
	args.control_condition+'.npy')
...
# Plot the V1 RSM
fig = plt.figure()
cax = plt.imshow(np.mean(rsm_v1, 0), cmap='RdBu_r', vmin=-1, vmax=1)
ax = plt.gca()
ax.set_xticks([])
ax.set_yticks([])
...
# Save the figure
fig_name = 'multivariate_rnc_' + args.control_condition + '_images_nsd_v1.svg'
fig.savefig(fig_name, bbox_inches='tight', transparent=True, format='svg',
	dpi=1000)

# Plot the V4 RSM
fig = plt.figure()
cax = plt.imshow(np.mean(rsm_v4, 0), cmap='RdBu_r', vmin=-1, vmax=1)
ax = plt.gca()
ax.set_xticks([])
ax.set_yticks([])
if args.control_condition == 'disentangle':
...
cbar.ax.yaxis.set_label_position('left')
# Save the figure
fig_name = 'multivariate_rnc_' + args.control_condition + '_images_nsd_v4.svg'
fig.savefig(fig_name, bbox_inches='tight', transparent=True, format='svg',
	dpi=1000)
...
fig = plt.figure()
plt.imshow(rsm_zoom, cmap='RdBu_r', vmin=-1, vmax=1)
...
ax.set_yticks([])
# Save the figure
fig_name = 'multivariate_rnc_' + args.control_condition + \
	'_images_nsd_v1_zoomed.svg'
fig.savefig(fig_name, bbox_inches='tight', transparent=True, format='svg',
	dpi=1000)
...
data_dir = os.path.join(args.project_dir, 'retinotopy_effect',
	...
	'rsms_control_condition-align.npy')
data = np.load(data_dir, allow_pickle=True).item()
...
# Save the figure
fig_name = 'multivariate_rnc_retinotopy_effect_aligning_images_pearson.svg'
fig.savefig(fig_name, bbox_inches='tight', transparent=True, format='svg')
...
for r, roi in enumerate(args.rois):
	...
	if data[roi]['sky_images_between_subject_pval'] < 0.05:
	...
	if data[roi]['non_sky_images_between_subject_pval'] < 0.05:
...
# Save the figure
fig_name = 'multivariate_rnc_retinotopy_effect_univariate_responses.svg'
fig.savefig(fig_name, bbox_inches='tight', transparent=True, format='svg')
