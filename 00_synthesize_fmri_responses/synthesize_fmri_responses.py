...
----------
subject : int
	Subject for which the fMRI responses are synthesized, out of all 8 (NSD)
	subjects.
roi : str
	ROI for which the fMRI responses are synthesized. Possible choices are 'V1',
	'V3', 'V2', 'hV4'.
imageset : str
	Image set for which the fMRI responses are synthesized. Possible choices are
	'nsd', 'imagenet_val', 'things'.
project_dir : str
...
print('>>> Synthesize fMRI responses <<<')
print('\nInput parameters:')
...
for i in tqdm(range(len(images))):
	...
	# Synthesize the fMRI image responses
	synthetic_fmri.append(np.squeeze(ned_object.encode(
		encoding_model,
		img,
...
synthetic_fmri = np.asarray(synthetic_fmri)
synthetic_fmri = synthetic_fmri.astype(np.float32)


# =============================================================================
# Save the synthetic fMRI responses
# =============================================================================
save_dir = os.path.join(args.project_dir, 'synthetic_fmri_responses',
	'imageset-'+args.imageset)

if os.path.isdir(save_dir) == False:
	os.makedirs(save_dir)

file_name = 'synthetic_fmri_responses_sub-' + format(args.subject, '02') + \
	'_roi-' + args.roi + '.h5'

# Save the h5py file
with h5py.File(os.path.join(save_dir, file_name), 'w') as f:
	f.create_dataset('synthetic_fmri_responses', data=synthetic_fmri,
		dtype=np.float32)
