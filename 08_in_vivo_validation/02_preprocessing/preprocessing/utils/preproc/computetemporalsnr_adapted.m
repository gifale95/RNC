function [tsnr,tsnr_mean] = computetemporalsnr(vols,dim)
...
if isempty(vols)
  ...
  mn = mean(vols,dim);
  valid = mn > 0.1 * prctile(flatten(mn),99);
  tsnr = mn./ ...
                reshape(std(projectionmatrix(constructpolynomialmatrix(size(vols,4),0:2))*squish(vols,3)',[],1),sizefull(mn,3));
  tsnr_mean = median(tsnr(valid));
end