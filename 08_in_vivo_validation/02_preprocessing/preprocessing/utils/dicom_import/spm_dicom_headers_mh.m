function hdr = spm_dicom_headers_mh(P, essentials)
% Read header information from DICOM files
% FORMAT hdr = spm_dicom_headers_mh(P [,essentials])
...
% Changed progress bar to text output 2010-08-27 M. Hebart
...
for i=1:size(P,1),
    ...
    if size(P,1)>1, msg_length = display_progress_ana(i,size(P,1),msg_length,msg); end;
end;
return;
%_______________________________________________________________________
