function out = spm_dicom_convert_mh(hdr,opts,root_dir,format)
% Convert DICOM images into something that SPM can use
% FORMAT spm_dicom_convert_mh(hdr,opts,root_dir,format)
% Inputs:
...
%
%
% Adapted by M. Hebart: hdr may contain additional field 'fname' which
% provides the full path of where the file should be written. The filename
% does not include the image type prefix 'f' or 's', this is added by this
% function. If field 'fname' is not provided, the files will be imported to
% pwd.

% Additional change: Progress bar replaced with progress text
...
msg_length = display_progress_ana(i,length(hdr),msg_length,msg);
end;
...
for i=1:length(hdr),
    ...
    volume(:,:,i) = plane;
%     spm_progress_bar('Set',i);
msg_length = display_progress_ana(i,length(hdr),msg_length);
end;
N.dat(:,:,:) = volume;
% spm_progress_bar('Clear');
return;
...
for i=1:length(hdr),
    ...
    elseif (~checkfields(hdr{i},'StartOfPixelData','SamplesperPixel',...
            ...
            ~checkfields(hdr{i},'StartOfPixelData','SamplesPerPixel',...
            'Rows','Columns','BitsAllocated','BitsStored','HighBit','PixelRepresentation'))
...
if isfield(hdr,'SamplesperPixel')
...
% %     end;
    ...
    %%%%% This part is added by M. Hebart):  %%%%%%%%
    % Change fname to prespecified fname and add filetype prefix
    
    if isfield(hdr,'fname')
        [fp,fn,fext] = fileparts(hdr.fname);
        fname = fullfile(fp,[prefix fn fext]);
                
    else
        fname = sprintf('%s%s-%.2d-%.3d.img', prefix, hdr.PatientsName, ...
            hdr.SeriesNumber, hdr.InstanceNumber); % MMMM changed to get rid of all the crap, PatientsName is the BCCN-standard subject ID
