...
for chunk = 1:cfg.sub(i_sub).n_betas
    ...
    psize = 1000; % size of parts of the matrix 
    count = 0;
    
    % this while clause splits up the matrix in smaller chunks to preserve
    % memory 
    while count < size(residuals,2)
    
    p = min(psize,size(residuals,2)-count);
    j = count+(1:p);
    fprintf('Chunking Matrix from %i to %i\n', j(1),j(end)); 
    % select only residuals from corresponding run
    these_residuals = residuals(residuals_chunk == chunk,j);
    ...
    for this_cat = 1: length(labelnames)
        ...
        beta_all(this_cat,chunk,j) = vol'*sigma^(-1/2);
    end 
    count = count+p; 
    end 
end 
