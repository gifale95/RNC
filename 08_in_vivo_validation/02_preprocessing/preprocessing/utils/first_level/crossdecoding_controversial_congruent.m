...
controversial_betas = cat(1,controversial_betas(1:15),controversial_betas(46:60)); 
...
% cfg.files.chunk = [repmat([1:5],1 ,60) repmat([6],1,30)]'; % little hack to pretend that all betas come from the same chunks (creates the right design) 
% % This creates the leave-one-run-out cross validation design:
% cfg.files.label = [ones(1,30*5), ones(1,30*5)*2 ones(1,15) ones(1,15)*2]';
% cfg.files.xclass = [ones(1,60*5) ones(1,30)*2];

cfg.files.chunk = [repmat([1:beta_nr],1 ,60) repmat([beta_nr+1],1,30)]'; % little hack to pretend that all betas come from the same chunks (creates the right design) 
% This creates the leave-one-run-out cross validation design:
cfg.files.label = [ones(1,30*beta_nr), ones(1,30*beta_nr)*2 ones(1,15) ones(1,15)*2]';
cfg.files.xclass = [ones(1,60*beta_nr) ones(1,30)*2];
