...
cfg.results.output = {'decision_values'}; 
...
cfg.files.name = controversial_betas; 
...
cfg.files.chunk = [ones(1,30) ones(1,30)*2]'; % little hack to pretend that all betas come from the same chunks (creates the right design) 
% This creates the leave-one-run-out cross validation design:
cfg.files.label = [ones(1,15), ones(1,30) ones(1,15)*2]';
cfg.files.xclass = [ones(1,15) ones(1,30)*2 ones(1,15)];
