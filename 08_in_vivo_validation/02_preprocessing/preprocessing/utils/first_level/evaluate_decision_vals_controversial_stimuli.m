...
if ismac
outdir = ['/Users/johannessinger/scratch/dfg_projekt/WP1/derived/pilot/',sub_id, '/results/decoding/crossdecoding_controversial_hrf_fitting/searchlight'];
...
elseif isunix 
    outdir = ['/scratch/singej96/dfg_projekt/WP1/derived/pilot/',sub_id, '/results/decoding/crossdecoding_controversial_hrf_fitting/searchlight'];
    roi_dir = ['/scratch/singej96/dfg_projekt/WP1/derived/pilot',sub_id, '/roi']; 
    load(fullfile(outdir,'res_decision_values.mat'));
end 
...
elseif isunix 
        template_file = dir(fullfile('/scratch/singej96/dfg_projekt/WP1/derived/pilot/',sub_id, '/alldata/run01','*.nii'));
end
...
resultsvol_hdr = hdr;
resultsvol_hdr.fname = fullfile(outdir,'congruent_controversial_difference.nii');
resultsvol_hdr.descrip = sprintf('Difference decision value map');
resultsvol = backgroundvalue * ones(resultsvol_hdr.dim(1:3)); % prepare results volume with background value (default: 0)
resultsvol(results.mask_index) = abs(manmade_manmade_vals-natural_natural_vals);
spm_write_vol(resultsvol_hdr,resultsvol);

%% evaluate results based on ROIs 

evc_mask = spm_read_vols(spm_vol(fullfile(roi_dir, 'evcmask.nii'))); 
IT_mask = spm_read_vols(spm_vol(fullfile(roi_dir, 'IT_mask.nii')));
