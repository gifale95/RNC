...
function results = decoding_nobetas(condition_names,avg_size,beta_dir,beta_avg_dir,out_dir,cfg,i_sub)
...
cfg.files.chunk = ones(1,length(condition_names); %repmat([1:avg_size],1,242)'; 
%
% (2) any numbers as class labels, normally we use 1 and -1. Each file gets a
% label number (i.e. a nx1 vector)
cfg.files.label = repelem([1:242],avg_size);
