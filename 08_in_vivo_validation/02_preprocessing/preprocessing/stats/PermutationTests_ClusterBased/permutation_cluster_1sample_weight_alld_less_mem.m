function [significantVarMax, significantVarWei, pValMax, pValWei, clusters,statmap] = permutation_cluster_1sample_weight_alld_less_mem (data, nperm, cluster_th, significance_th, tail)
...
if ~exist('tail','var') || strcmp(tail,'right')
    func = '';
elseif strcmp(tail,'left') % if left-sided
    func = '-'; 
...
% create statmap variable for visually inspecting the uncorrected p-vals
statmap = squeeze(StatMapPermPV(1,cln{:})); 
